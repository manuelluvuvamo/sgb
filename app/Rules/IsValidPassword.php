...
class IsValidPassword implements Rule, DataAwareRule, ValidatorAwareRule
{
    use Conditionable;
    ...
    public function passes($attribute, $value)
    {
        ...
        $validator = Validator::make(
            ...
            if ($this->mixedCase && ! preg_match('/(\p{Ll}+.*\p{Lu})|(\p{Lu}+.*\p{Ll})/u', $value)) {
                $validator->errors()->add($attribute, 'A senha deve conter pelo menos uma letra em maiúsculo e em minúsculo.');
            }
            ...
            if ($this->symbols && ! preg_match('/\p{Z}|\p{S}|\p{P}/u', $value)) {
                $validator->errors()->add($attribute, 'A senha deve conter pelo menos um caractere especial.');
            }

            if ($this->numbers && ! preg_match('/\pN/u', $value)) {
                $validator->errors()->add($attribute, 'A senha deve conter pelo menos um número');
            }
        });
